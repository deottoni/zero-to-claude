...
**Admin-only trigger:** if the person says something like "run the demo," "let's demo this," or "demo mode" instead of answering normally, stop here — don't run Phase 0 below. Read `../demo/README.md` (one level up from this file, at the repo root) and follow it instead; it's a self-contained alternate flow for Andre to rehearse or present with, not something a real client would ever ask for.
