...
1. **An agent** — a persona/perspective Claude inhabits for that work. Name, short description, perspective, standing instructions. Opinionated and useful, not generic.
...
**Files mode:** write directly — agents → `.claude/agents/[use-case].md`, skills → `.claude/skills/[use-case].md`, templates → `templates/[use-case].md`.
**Text-blocks mode:** output each as a formatted block to copy into Project Instructions or save separately.

Reference (guidance, not a checklist — adapt to what this person actually does):
...
After generating, show a clean summary — names and one-line descriptions of each agent, skill, template. Then say:
...
**Test 2 — The Agent.** Invoke the most relevant agent you built. *"I built an advisor who thinks like [perspective]. Let me show you."* Step into that voice for a realistic scenario. Then: *"That's your [agent name]. Notice anything different?"*
